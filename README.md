...
- Write a **POST api /users** to register a user from the user details in request body. //DONE
...
On successful login, generate a JWT token and return it in response body. //DONE
 Example 
```
{
    status: true,
...
If present, check that the token is valid.//DONE

- Write a **PUT api /users/:userId** to update user details. Pass the userId as path param in the url and update the attributes received in the request body. Check that request must contain **x-auth-token** header. If absent, return a suitable error.//DONE
